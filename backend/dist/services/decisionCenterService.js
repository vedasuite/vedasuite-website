...
exports.getUnifiedDecisionCenter = getUnifiedDecisionCenter;
const prismaClient_1 = require("../db/prismaClient");
async function getUnifiedDecisionCenter(shopDomain) {
    ...
    const [highRiskOrder, riskyCustomer, competitorSignal, pricingMove, profitMove] = await Promise.all([
        prismaClient_1.prisma.order.findFirst({
            where: { storeId: store.id },
            orderBy: [{ fraudScore: "desc" }, { createdAt: "desc" }],
        }),
        prismaClient_1.prisma.customer.findFirst({
            where: { storeId: store.id },
            orderBy: [{ creditScore: "asc" }, { refundRate: "desc" }],
        }),
        prismaClient_1.prisma.competitorData.findFirst({
            where: { storeId: store.id },
            orderBy: { collectedAt: "desc" },
        }),
        prismaClient_1.prisma.priceHistory.findFirst({
            where: { storeId: store.id },
            orderBy: { createdAt: "desc" },
        }),
        prismaClient_1.prisma.profitOptimizationData.findFirst({
            where: { storeId: store.id },
    ...
    if (highRiskOrder) {
        decisions.push({
            ...
            recommendedAction: highRiskOrder.fraudScore >= 85 ? "Block or send to review" : "Manual review",
            explanationPoints: [
            ...
            automationPosture: highRiskOrder.fraudScore >= 85
    ...
    if (riskyCustomer) {
        decisions.push({
            ...
            confidence: Math.max(48, Math.min(94, 100 - riskyCustomer.creditScore + Math.round(riskyCustomer.refundRate * 35))),
            recommendedAction: riskyCustomer.creditScore < 50
            ...
            automationPosture: riskyCustomer.creditScore < 50
    ...
    if (pricingMove) {
        decisions.push({
            ...
            severity: (pricingMove.expectedProfitGain ?? 0) >= 100 ? "High" : "Medium",
            ...
            confidence: Math.max(56, Math.min(95, Math.round(62 +
    ...
    if (profitMove) {
        decisions.push({
            ...
            module: "Pricing & Profit",
            severity: (profitMove.projectedMonthlyProfit ?? 0) >= 1000 ? "High" : "Medium",
            ...
            confidence: Math.max(54, Math.min(94, 60 + Math.round((profitMove.projectedMonthlyProfit ?? 0) / 70))),
    ...
    return {
        summary: {
            ...
            priorityLevel: decisions.some((decision) => decision.severity === "High")
                ? "High"
                : "Medium",
            automationReadiness: decisions.some((decision) => decision.automationPosture.toLowerCase().includes("automation"))
