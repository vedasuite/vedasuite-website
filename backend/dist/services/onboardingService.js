...
exports.selectOnboardingModule = selectOnboardingModule;
exports.markOnboardingInsightViewed = markOnboardingInsightViewed;
exports.confirmOnboardingPlan = confirmOnboardingPlan;
exports.markOnboardingComplete = markOnboardingComplete;
exports.dismissOnboarding = dismissOnboarding;
...
function normalizeOnboardingModule(value) {
...
function mapDashboardState(syncStatus) {
    switch (syncStatus) {
        case "NOT_CONNECTED":
...
function firstIncompleteIndex(steps) {
    const index = steps.findIndex((step) => !step.complete);
    return index === -1 ? steps.length - 1 : index;
}
async function getOnboardingState(shopDomain) {
    const [storeResult, connection, operational, billing, subscription, readiness] = await Promise.all([
        ...
        (0, readinessEngineService_1.getUnifiedReadinessState)(shopDomain),
    ]);
    ...
    const hasAnyRawData = operational.counts.products + operational.counts.orders + operational.counts.customers > 0;
    const hasAnyProcessedData = operational.counts.pricingRows +
        operational.counts.profitRows +
        operational.counts.timelineEvents +
        operational.counts.competitorRows >
        0;
    const webhooksReady = !!store.webhooksRegisteredAt &&
        store.lastWebhookRegistrationStatus !== "FAILED";
    const selectedModule = normalizeOnboardingModule(store.onboardingSelectedModule) ??
    ...
    const selectedModuleAvailable = !!selectedModule &&
        moduleAvailability.some((module) => module.key === selectedModule && module.available);
    const selectedModuleReadiness = selectedModule === "fraud"
    ...
    const firstInsightViewedComplete = moduleSelectionComplete &&
        !!selectedModuleReadiness?.ready &&
        !!store.onboardingFirstInsightViewedAt;
    const planConfirmationComplete = readiness.billing.ready && !!store.onboardingPlanConfirmedAt;
    const canAccessDashboard = readiness.setup.minimumComplete &&
    ...
    const stepTemplates = [
        {
            ...
            description: "Sync live Shopify products, customers, and orders so VedaSuite can analyze the store.",
        ...
        },
        {
            ...
            description: "Pick one module to start with so the first guided workflow is clear and focused.",
            helper: !readiness.initialSync.ready
        ...
        },
        {
            ...
            description: "Open the selected module and review the first real store insight before moving into the dashboard.",
            helper: !moduleSelectionComplete
        ...
        },
        {
            ...
            description: "Confirm the current plan so VedaSuite can unlock the right modules and take you to the dashboard.",
    ...
    const steps = stepTemplates.map((step, index) => ({
        ...step,
    ...
    const primaryAction = !readiness.connection.healthy
        ? {
            key: "RECONNECT_SHOPIFY",
        ...
        : stage === "DATA_SYNC"
            ? {
                key: "SYNC_LIVE_DATA",
                label: readiness.initialSync.state === "collecting_data" ? "Preparing results" : "Sync Data",
                route: "/app/onboarding",
            }
    ...
    const stateSummary = readiness.setup.minimumComplete
        ? {
    ...
    return {
        ...
        description: "VedaSuite turns Shopify orders, customers, and products into fraud detection, competitor tracking, and pricing guidance for your store.",
        primaryAction,
        progress: {
            completedSteps: stepTemplates.filter((step) => step.complete).length,
            totalSteps: stepTemplates.length,
            percent: Math.round((stepTemplates.filter((step) => step.complete).length / stepTemplates.length) * 100),
        },
        steps,
        hero: {
            headline: "Turn Your Store Data Into Fraud Detection & Profit Insights",
            subtext: "VedaSuite syncs Shopify data, detects refund and fraud abuse, tracks competitor pricing and ads, and surfaces pricing opportunities that protect profit.",
            benefits: [
        ...
        guidedInsights: env_1.env.enableGuidedSetupData
            ? [
                {
                    key: "fraud-guided",
                    module: "Fraud Intelligence",
                    title: "Guided setup: Customer flagged for repeated refund behaviour",
                    detail: "Fraud insights appear here after Shopify orders and customer history are available.",
                },
                {
                    key: "competitor-guided",
                    module: "Competitor Intelligence",
                    title: "Guided setup: Competitor changed price on a tracked product",
                    detail: "Competitor changes appear after competitor websites are connected and analysis completes.",
                },
                {
                    key: "pricing-guided",
                    module: "AI Pricing Engine",
                    title: "Guided setup: Suggested price change based on baseline store data",
                    detail: "Pricing actions appear after enough product and order history is available.",
                },
            ]
            : [],
        planSummary: {
            ...
            starterModule: (0, capabilities_1.normalizeStarterModuleLabel)(subscription.starterModule) ??
                null,
            unlockedFeatures: [
        ...
        privacySummary: {
            ...
            description: "VedaSuite accesses Shopify orders, customers, and products to generate insights inside the app.",
            bullets: [
        ...
        billingActive: readiness.billing.accessActive,
        limitedDataReason: syncState.status === "EMPTY_STORE_DATA"
...
async function selectOnboardingModule(input) {
...
async function markOnboardingInsightViewed(input) {
...
async function confirmOnboardingPlan(shopDomain) {
...
async function markOnboardingComplete(shopDomain) {
    const onboarding = await getOnboardingState(shopDomain);
...
async function dismissOnboarding(shopDomain) {
    await prismaClient_1.prisma.store.update({
        where: { shop: shopDomain },
        data: {
