...
exports.getPricingProfitOverview = getPricingProfitOverview;
const prismaClient_1 = require("../db/prismaClient");
const competitorService_1 = require("./competitorService");
const observabilityService_1 = require("./observabilityService");
const pricingEngineStateService_1 = require("./pricingEngineStateService");
const pricingService_1 = require("./pricingService");
const profitService_1 = require("./profitService");
...
const SHOPIFY_DEMO_PRODUCT_PATTERN = /\bsnowboard\b|\bgift[-\s]?card\b|\bthe-collection\b|\bhydrogen\b/i;
function isShopifyDemoProduct(handle) {
    return SHOPIFY_DEMO_PRODUCT_PATTERN.test(handle);
}
function deriveRecommendationAction(currentPrice, recommendedPrice) {
...
function derivePricingConfidence(args) {
    ...
    if (args.approvalConfidence >= 70)
        return "High";
    if (args.approvalConfidence >= 52)
        return "Medium";
    return "Baseline estimate";
}
async function safelyResolve(work, fallback) {
    try {
        return await work;
    }
    catch {
...
async function safelyResolveWithTimeout(work, fallback, timeoutMs = 8000) {
    return safelyResolve(Promise.race([
        work,
...
async function resolveWithTimeoutReport(work, fallback, timeoutMs = 8000) {
    let timedOut = false;
    const value = await safelyResolve(Promise.race([
        work,
        new Promise((_, reject) => {
            setTimeout(() => {
                timedOut = true;
...
function isValidPricingRecommendation(value) {
    return (value &&
...
async function getPricingProfitOverview(shopDomain) {
    const startedAt = Date.now();
    (0, observabilityService_1.logEvent)("info", "pricing_profit.overview_started", { shop: shopDomain });
    try {
        ...
        const subscription = await (0, subscriptionService_1.getCurrentSubscription)(shopDomain);
        ...
        const [pricingRecommendationResult, competitorResponseResult] = await Promise.all([
            resolveWithTimeoutReport((0, pricingService_1.getPricingRecommendations)(shopDomain), [], 7000),
            resolveWithTimeoutReport((0, competitorService_1.getCompetitorResponseEngine)(shopDomain), {
                summary: {
                    responseMode: "Awaiting monitored competitor data",
                    topPressureCount: 0,
                    automationReadiness: "Competitor response guidance appears after monitored domains are configured and live observations are collected.",
                },
                responsePlans: [],
            }, 7000),
        ]);
        ...
        const invalidRecommendationCount = rawPricingRecommendations.filter((item) => !isValidPricingRecommendation(item)).length;
        const pricingRecommendations = rawPricingRecommendations.filter(isValidPricingRecommendation);
        const competitorDependencyStatus = operational.counts.competitorRows > 0 ? "ready" : "missing";
        const pricingDependencyStatus = operational.counts.pricingRows + operational.counts.profitRows > 0
            ? "ready"
            : "missing";
        const fraudDependencyStatus = operational.counts.timelineEvents > 0 ? "ready" : "missing";
        ...
        const canUseScenarioSimulator = subscription.capabilities["pricing.scenarioSimulator"];
        const canUseDailyActionBoard = subscription.capabilities["pricing.dailyActionBoard"];
        const canUseMarginAtRisk = subscription.capabilities["pricing.marginAtRisk"];
        const canUseProfitLeakDetector = subscription.capabilities["pricing.profitLeakDetector"];
        const canUseExplainablePricing = subscription.capabilities["pricing.explainableRecommendations"];
        const profitOpportunities = canUseFullProfitEngine
            ? await safelyResolveWithTimeout((0, profitService_1.getProfitOpportunities)(shopDomain), [], 7000)
            : [];
        ...
        const scenarioPreset = topRecommendation
            ? await safelyResolveWithTimeout((0, pricingService_1.simulatePricingChange)({
                currentPrice: topRecommendation.currentPrice,
                recommendedPrice: topRecommendation.recommendedPrice,
                salesVelocity: typeof topRecommendation.demandScore === "number"
        ...
        const dailyActionBoard = [
            ...
            },
            {
                ...
                title: competitorResponse.summary.responseMode === "Hold and monitor"
                ...
                actionType: "market",
                priority: competitorResponse.summary.responseMode === "Hold and monitor"
                    ? "Medium"
                    : "High",
                expectedImpact: competitorResponse.summary.topPressureCount > 0
        ...
        const pricingModes = [
            {
                ...
                description: "Protect contribution margin and approve only the strongest price changes.",
                available: canUseAdvancedModes,
                recommended: canUseAdvancedModes &&
            ...
            },
            {
                key: "balanced",
                label: "Balanced",
                description: "Blend conversion support with controlled margin protection across the catalog.",
                available: true,
                recommended: subscription.capabilities["pricing.basicRecommendations"] ||
            ...
            },
            {
                ...
                description: "Lean into conversion support when demand expansion matters more than short-term margin.",
                available: canUseAdvancedModes,
                recommended: competitorResponse.summary.responseMode === "Respond selectively",
                gate: canUseAdvancedModes ? "Included" : "Pro",
            },
            {
                key: "defend-market-share",
                label: "Defend market share",
                description: "Use targeted pricing responses on exposed SKUs while preserving margin guardrails.",
                available: canUseAdvancedModes,
                recommended: competitorResponse.summary.responseMode === "Respond selectively" &&
            ...
            },
            {
                ...
                description: "Bias toward sell-through on slower SKUs with inventory drag or promo pressure.",
                available: canUseAdvancedModes,
                recommended: false,
                gate: canUseAdvancedModes ? "Included" : "Pro",
            },
            {
                ...
                description: "Hold price and emphasize value when competitor pressure is low and margin matters most.",
                available: canUseAdvancedModes,
                recommended: canUseAdvancedModes &&
                    competitorResponse.summary.responseMode === "Hold and monitor",
                gate: canUseAdvancedModes ? "Included" : "Pro",
        ...
        const doNothingRecommendation = recommendationCount === 0
            ? {
                headline: "No recommendation yet",
                rationale: "VedaSuite needs more synced pricing and margin data before it can justify a no-change recommendation.",
            }
            : topRecommendation &&
                Math.abs(topRecommendation.recommendedPrice - topRecommendation.currentPrice) < 1
                ? {
                    headline: "Do nothing on low-delta SKUs",
                    rationale: "Some products are close enough to target price that reacting now would add noise without meaningful profit lift.",
                }
                : null;
        const profitLeakSummary = [
            {
                ...
                severity: recommendationCount >= 6 ? "High" : recommendationCount >= 2 ? "Medium" : "Low",
                action: recommendationCount > 0
                    ? "Review approval-led pricing actions"
                    : "Run live sync before reviewing pricing posture",
            },
            {
                title: "Return-linked margin pressure",
                detail: canUseProfitLeakDetector && profitOpportunityCount > 0
                ...
                severity: profitOpportunityCount >= 4 ? "High" : profitOpportunityCount >= 1 ? "Medium" : "Low",
            ...
            },
            {
                ...
                detail: competitorResponse.summary.topPressureCount > 0
                ...
                severity: competitorResponse.summary.topPressureCount >= 4
                    ? "High"
                    : competitorResponse.summary.topPressureCount >= 1
                        ? "Medium"
                        : "Low",
                action: competitorResponse.summary.topPressureCount > 0
        ...
        const scenarioPlaybook = [
            {
                scenario: "Hold price",
                outcome: competitorResponse.summary.responseMode === "Hold and monitor"
            ...
            },
            {
                ...
                outcome: competitorResponse.summary.responseMode === "Respond selectively"
            ...
            },
            {
                ...
                outcome: competitorResponse.summary.topPressureCount > 0
        ...
        const explainabilityHighlights = pricingRecommendations.slice(0, 4).map((item) => ({
            ...
            recommendation: item.recommendedPrice > item.currentPrice ? "Increase price" : "Reduce price",
            why: item.demandSignals[0] ??
            ...
            guardrail: item.autoApprovalCandidate
            ...
            ? await Promise.all([
                {
                    ...
                    summary: "Keep the current price and avoid reacting until pressure becomes stronger.",
                },
                {
                    id: "match",
                    title: "Selective response",
                    recommendedPrice: topRecommendation.recommendedPrice,
                    summary: "Follow the current recommendation on exposed SKUs only.",
                },
                {
                    id: "lift",
                    title: "Margin defense",
                    recommendedPrice: Number((topRecommendation.currentPrice + 2).toFixed(2)),
                    summary: "Protect margin by holding or slightly increasing price where live demand and competitor pressure appear limited.",
                },
            ].map(async (scenario) => {
                const result = await safelyResolveWithTimeout((0, pricingService_1.simulatePricingChange)({
                    currentPrice: topRecommendation.currentPrice,
                ...
                return {
                    ...scenario,
                    projectedMonthlyProfitGain: result?.projectedMonthlyProfitGain ?? 0,
                    expectedMarginImprovement: result?.expectedMarginImprovement ?? 0,
        ...
        const marginRiskDrivers = [
            ...
            },
            {
                ...
                detail: competitorResponse.responsePlans[0]?.promotionSignals
                ...
                severity: (competitorResponse.responsePlans[0]?.promotionSignals ?? 0) >= 3
                    ? "High"
        ...
        const lastSuccessfulSyncAt = (0, unifiedModuleStateService_1.toIsoString)(operational.latestProcessingAt ?? operational.store.lastSyncAt);
        const lastAttemptAt = (0, unifiedModuleStateService_1.toIsoString)(operational.latestSyncJob?.finishedAt ?? operational.latestSyncJob?.startedAt);
        const hasPricingData = recommendationCount > 0 || profitOpportunityCount > 0 || operational.counts.pricingRows > 0;
        const moduleState = syncState.status === "SYNC_IN_PROGRESS"
            ? (0, unifiedModuleStateService_1.createUnifiedModuleState)({
                ...
                description: "VedaSuite is updating pricing recommendations and profitability insights.",
            ...
            : syncState.status === "FAILED" || readiness.readinessState === "FAILED"
                ...
                : (0, unifiedModuleStateService_1.isStaleTimestamp)(operational.latestProcessingAt ?? operational.store.lastSyncAt)
                    ? (0, unifiedModuleStateService_1.createUnifiedModuleState)({
                        setupStatus: "complete",
                        ...
                        description: "Run a new analysis to review current recommendations.",
                    ...
                    : !hasPricingData && syncState.status === "EMPTY_STORE_DATA"
                        ? (0, unifiedModuleStateService_1.createUnifiedModuleState)({
                            ...
                            description: "No important pricing changes are recommended from the current store activity.",
                        ...
                        : hasPricingData && competitorDependencyStatus === "missing"
                            ? (0, unifiedModuleStateService_1.createUnifiedModuleState)({
                                ...
                                description: "Baseline pricing recommendations are ready. Competitor-informed comparisons will improve after competitor websites are analyzed.",
                            ...
                            : hasPricingData
                                ? (0, unifiedModuleStateService_1.createUnifiedModuleState)({
                                    ...
                                    description: "VedaSuite has prepared pricing recommendations and profit guidance based on the latest store data.",
                                ...
                                : (0, unifiedModuleStateService_1.createUnifiedModuleState)({
                                    ...
                                    description: "VedaSuite needs synced product, order, and pricing data before it can generate pricing guidance.",
        ...
        const projectedGainValue = profitReady
            ? topProfitOpportunity?.projectedMonthlyProfitGain ??
                ...
                0
            : 0;
        const projectedGainStatus = projectedGainValue > 0 && profitReady ? "available" : "not_available";
        const primaryState = moduleState.dataStatus === "processing"
            ? "PROCESSING"
                ...
                : moduleState.setupStatus === "incomplete"
                    ...
                    : recommendationCount === 0 && profitOpportunityCount === 0
                        ...
                        : moduleState.dataStatus === "partial"
                            ? "PARTIAL_READINESS"
                            : "READY";
        const responseMode = !competitorReady && !profitReady
            ? "baseline_only"
        ...
        const prioritizedRecommendations = pricingRecommendations
            ...
            const actionLabel = deriveRecommendationAction(item.currentPrice, item.recommendedPrice);
            ...
            const expectedImpact = exampleCatalogProduct
            ...
            return {
                ...
                inputsUsed,
                merchantActionNote: item.autoApprovalCandidate
        ...
        const diagnosticSummary = [
            {
                ...
                detail: topRecommendation?.demandTrend && topRecommendation.demandTrend !== "insufficient history"
                ...
                status: topRecommendation?.demandTrend && topRecommendation.demandTrend !== "insufficient history"
                    ? "ready"
                    : "partial",
            },
            {
            ...
            },
            {
                ...
                status: competitorResponse.summary.topPressureCount > 0 ? "ready" : "empty",
            },
            {
                title: "Profit leakage",
                detail: profitReady
        ...
        const planGateSummary = [
            ...
            },
            {
                ...
                detail: "Pro unlocks advanced modes, profit leakage workflows, scenario simulation, and deeper margin defense.",
            },
        ];
        const viewState = (0, pricingEngineStateService_1.derivePricingEngineViewState)({
            syncStatus: syncState.status,
            moduleState,
        ...
        (0, observabilityService_1.logEvent)("info", "pricing_profit.overview_resolved", {
            shop: shopDomain,
        ...
        return {
            ...
            pricingState: {
                ...
                pricingStatus: moduleState.dataStatus === "processing"
                    ? "processing"
                    : moduleState.dataStatus === "failed"
                        ? "failed"
                ...
                description: primaryState === "PARTIAL_READINESS"
            ...
            summary: {
                ...
                responseMode: responseMode === "baseline_only"
                ...
                automationReadiness: moduleState.dataStatus === "partial"
            ...
            marginAtRisk: {
                pressureProducts: competitorResponse.responsePlans
                    ...
                    .slice(0, 5),
                projectedMonthlyGain: topProfitOpportunity?.projectedMonthlyProfitGain ??
                    topRecommendation?.expectedProfitGain ??
                    0,
                summary: competitorResponse.summary.topPressureCount > 0
        ...
        (0, observabilityService_1.logEvent)("error", "pricing_profit.overview_failed", {
            shop: shopDomain,
            durationMs: Date.now() - startedAt,
            error,
        });
