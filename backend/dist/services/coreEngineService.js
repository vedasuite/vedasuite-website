...
exports.recomputeStoreDerivedData = recomputeStoreDerivedData;
...
function calculateTrustScore(customer) {
    ...
    const score = clamp(Math.round(58 +
        Math.min(18, ordersCount * 2.5) +
        Math.min(10, avgOrderValue / 40) +
        paymentReliability -
        refundRate * 42 -
        recentSignals * 8), 0, 100);
    const category = score >= 80 ? "Trusted Buyer" : score >= 55 ? "Standard Buyer" : "Review Buyer";
    const reasons = [];
    if (ordersCount <= 1) {
...
function calculateReturnAbuseScore(customer) {
    const ordersCount = customer.orders.length;
    const refunds = customer.orders.filter((order) => order.refunded || order.refundRequested).length;
    const refundRate = ordersCount > 0 ? refunds / ordersCount : customer.refundRate;
    const quickRefundSignals = customer.orders.filter((order) => (order.refunded || order.refundRequested) &&
        Date.now() - order.createdAt.getTime() <= 14 * 24 * 60 * 60 * 1000).length;
    const score = clamp(Math.round(refundRate * 65 +
        quickRefundSignals * 8 +
        customer.fraudSignals.length * 6 +
        (ordersCount >= 4 && refunds >= 2 ? 10 : 0)), 0, 100);
    const reasons = [];
    if (refundRate >= 0.4) {
...
function baselinePriceRecommendation(args) {
    ...
    const recommendedPrice = roundMoney(Math.max(1, args.currentPrice +
...
function buildTimelineEvents(store) {
    const events = [];
    ...
    for (const order of store.orders.slice(0, 25)) {
        ...
        const orderLabel = (0, merchantLabels_1.getMerchantOrderLabelOrNull)(order);
        events.push({
            storeId: store.id,
...
async function recomputeStoreDerivedData(shopDomain) {
    ...
    const customerUpdates = store.customers.map((customer) => {
        const trust = calculateTrustScore(customer);
        return prismaClient_1.prisma.customer.update({
    ...
    const baselineProducts = new Set();
    const pricingCreates = [];
    const profitCreates = [];
    ...
    const storeReturnRate = store.orders.length > 0
        ? store.orders.filter((order) => order.refunded || order.refundRequested).length /
            store.orders.length
        : 0.08;
    for (const productHandle of baselineProducts) {
        const latestPrice = store.priceHistory.find((row) => row.productHandle === productHandle) ?? null;
        const latestProfit = store.profitData.find((row) => row.productHandle === productHandle) ?? null;
        const competitorRows = store.competitorData.filter((row) => row.productHandle === productHandle);
        const currentPrice = latestPrice?.currentPrice ??
            latestProfit?.sellingPrice ??
            roundMoney(competitorRows.find((row) => row.price != null)?.price ?? 49);
        const competitorAveragePrice = competitorRows.filter((row) => row.price != null).length > 0
            ? roundMoney(competitorRows
        ...
        const expectedProfitGain = roundMoney(Math.max(0, recommendedPrice - currentPrice) * (latestProfit?.salesVelocity ?? 8) * 6);
        if (!latestPrice || Math.abs(latestPrice.recommendedPrice - recommendedPrice) > 0.01) {
            pricingCreates.push(prismaClient_1.prisma.priceHistory.create({
                data: {
                    storeId: store.id,
                    productHandle,
                    currentPrice,
                    ...
                    rationaleJson: JSON.stringify({
                        ...
                        demandScore: clamp(Math.round((latestProfit?.salesVelocity ?? 8) * 5 + (100 - store.profitGuardrail)), 25, 95),
                        demandTrend: (latestProfit?.salesVelocity ?? 8) >= 14
                            ? "strong"
                        ...
                        competitorPressure: competitorAveragePrice != null && competitorAveragePrice < currentPrice
                            ? "high"
                            : competitorAveragePrice != null
                                ? "medium"
                                : "baseline_only",
        ...
        const optimalPrice = roundMoney(Math.max(currentPrice, recommendedPrice + currentPrice * (store.profitGuardrail / 1000)));
        const projectedMarginIncrease = roundMoney(((optimalPrice - currentPrice) / currentPrice) * 100);
        const projectedMonthlyProfit = roundMoney(Math.max(0, optimalPrice - productCost - (latestProfit?.shippingCost ?? currentPrice * 0.06) - (latestProfit?.advertisingSpend ?? currentPrice * 0.1)) *
            salesVelocity *
            4);
        if (!latestProfit || Math.abs((latestProfit.optimalPrice ?? 0) - optimalPrice) > 0.01) {
            profitCreates.push(prismaClient_1.prisma.profitOptimizationData.create({
                data: {
                    storeId: store.id,
                    productHandle,
                    productCost,
                    ...
                    discountStrategyJson: JSON.stringify({
                        fallbackUsed: competitorAveragePrice == null,
                        strategy: competitorAveragePrice != null && competitorAveragePrice < currentPrice
                            ? "Selective response"
                            : "Hold and monitor",
    ...
    const fraudSignalsGenerated = timelineEvents.filter((event) => event.category === "abuse" || event.severity === "critical").length;
    await prismaClient_1.prisma.$transaction([
        ...customerUpdates,
        ...orderUpdates,
        ...pricingCreates,
        ...profitCreates,
        prismaClient_1.prisma.timelineEvent.deleteMany({ where: { storeId: store.id } }),
        ...timelineEvents.map((event) => prismaClient_1.prisma.timelineEvent.create({ data: event })),
    ]);
    (0, observabilityService_1.logEvent)("info", "core_engine.recomputed", {
        shop: shopDomain,
