...
exports.deriveReadinessState = deriveReadinessState;
exports.getUnifiedReadinessState = getUnifiedReadinessState;
...
function toQuickAccessStatus(state) {
    switch (state) {
        case "locked":
            return "Locked";
        case "setup_needed":
...
function readinessStateToModuleStatus(state) {
    switch (state) {
        case "ready":
            return "ready";
        case "collecting_data":
...
function buildModuleStateFromReadiness(input) {
    return (0, unifiedModuleStateService_1.createUnifiedModuleState)({
...
function syncStatusToCanonicalState(syncStatus) {
    switch (syncStatus) {
...
function buildSetupSummary(input) {
    ...
    const nextAction = !input.connection.ready
    ...
    const allCoreModulesReady = input.fraud.ready && input.competitor.ready && input.pricing.ready;
    const minimumComplete = input.connection.ready &&
        input.sync.ready &&
        input.billing.ready &&
        input.selectedModuleState === "ready";
...
function normalizeSelectedModule(value) {
...
async function getUnifiedReadinessState(shopDomain) {
    const [connectionHealth, operational, billing, subscription] = await Promise.all([
        (0, shopifyConnectionService_1.getConnectionHealth)(shopDomain, { probeApi: false }),
    ...
    const syncStatus = (0, storeOperationalStateService_1.deriveSyncStatus)({
    ...
    const hasRawData = operational.counts.products + operational.counts.orders + operational.counts.customers > 0;
    const hasProcessedData = operational.counts.pricingRows +
        operational.counts.profitRows +
        operational.counts.timelineEvents +
        operational.counts.competitorRows >
        0;
    const lastProcessingAt = (0, unifiedModuleStateService_1.toIsoString)(operational.latestProcessingAt);
    const lastCompetitorAt = (0, unifiedModuleStateService_1.toIsoString)(operational.latestCompetitorAt);
    const lastSyncAttemptAt = (0, unifiedModuleStateService_1.toIsoString)(operational.latestSyncJob?.finishedAt ?? operational.latestSyncJob?.startedAt ?? null);
    const lastCompetitorAttemptAt = (0, unifiedModuleStateService_1.toIsoString)(operational.latestCompetitorIngestJob?.finishedAt ??
    ...
    const initialSync = createReadinessItem({
        ...
        title: syncStatus.status === "READY_WITH_DATA"
        ...
        nextAction: initialSyncState === "ready"
            ? "Continue setup"
            : initialSyncState === "error"
                ? "Try again"
                : "Update store insights",
        route: "/app/onboarding",
        freshnessAt: (0, unifiedModuleStateService_1.toIsoString)(operational.store.lastSyncAt),
        detail: {
            products: operational.counts.products,
            orders: operational.counts.orders,
            customers: operational.counts.customers,
    ...
    const billingState = billing.lifecycle === "active"
        ? "ready"
    ...
    const fraudReadiness = createReadinessItem({
        ...
        title: !subscription.enabledModules.fraud
        ...
        description: !subscription.enabledModules.fraud
        ...
        nextAction: !subscription.enabledModules.fraud
            ? "Open billing"
            : operational.counts.timelineEvents > 0
    ...
    const competitorCollecting = operational.latestCompetitorIngestJob?.status === "RUNNING" ||
    ...
    const competitorDescription = !subscription.enabledModules.competitor
        ...
        : !competitorHasSetup
            ...
            : competitorFailed
                ...
                : operational.counts.competitorRows > 0
                    ? (0, unifiedModuleStateService_1.isStaleTimestamp)(operational.latestCompetitorAt)
    ...
    const competitorReadiness = createReadinessItem({
        state: competitorState === "ready" && (0, unifiedModuleStateService_1.isStaleTimestamp)(operational.latestCompetitorAt)
            ? "collecting_data"
            : competitorState,
        title: !subscription.enabledModules.competitor
            ? "Competitor Intelligence is locked"
            : !competitorHasSetup
                ? "Add competitor websites to begin analysis"
                : operational.counts.competitorRows > 0 && !(0, unifiedModuleStateService_1.isStaleTimestamp)(operational.latestCompetitorAt)
                    ? "Competitor analysis is active"
                    : competitorCollecting
                        ? "Competitor analysis is updating"
                        : "Add competitor websites to begin analysis",
        description: competitorDescription,
        nextAction: !subscription.enabledModules.competitor
            ? "Open billing"
            : !competitorHasSetup
    ...
    const provisionalPricingModuleState = (0, unifiedModuleStateService_1.createUnifiedModuleState)({
        ...
        syncStatus: syncStatus.status === "FAILED"
            ...
            : syncStatus.status === "SYNC_IN_PROGRESS" ||
                ...
                ? "running"
                : "completed",
        dataStatus: operational.counts.pricingRows + operational.counts.profitRows > 0
            ? "ready"
            : syncStatus.status === "SYNC_IN_PROGRESS" ||
                syncStatus.status === "SYNC_COMPLETED_PROCESSING_PENDING"
                ? "processing"
        ...
        dependencies: {
            ...
            pricing: operational.counts.pricingRows + operational.counts.profitRows > 0
                ? "ready"
                : "missing",
        },
    ...
    const pricingViewState = (0, pricingEngineStateService_1.derivePricingEngineViewState)({
        syncStatus: syncStatus.status,
    ...
    const pricingReadiness = createReadinessItem({
        state: !subscription.enabledModules.pricing
            ? "locked"
            : pricingViewState.status === "ready"
                ...
                : pricingViewState.status === "failed_timeout" ||
                    ...
                    : pricingViewState.status === "syncing"
                        ? "collecting_data"
                        : "setup_needed",
        title: !subscription.enabledModules.pricing
        ...
        nextAction: !subscription.enabledModules.pricing
            ? "Open billing"
            : pricingViewState.status === "ready"
    ...
    const moduleStates = {
        fraud: buildModuleStateFromReadiness({
            ...
            syncStatus: fraudReadiness.state === "error"
                ? "failed"
        ...
        competitor: buildModuleStateFromReadiness({
            ...
            syncStatus: competitorReadiness.state === "error"
                ? "failed"
            ...
            coverage: operational.counts.competitorRows > 0
                ? "full"
                : competitorHasSetup
                    ? "partial"
                    : "none",
        ...
        pricing: buildModuleStateFromReadiness({
            ...
            syncStatus: pricingReadiness.state === "error"
                ? "failed"
            ...
            lastAttemptAt: lastSyncAttemptAt,
            coverage: operational.counts.pricingRows + operational.counts.profitRows > 0
                ? operational.counts.competitorRows > 0
                    ? "full"
                    : "partial"
                : "none",
            ...
            dataChanged: operational.counts.pricingRows + operational.counts.profitRows > 0,
        }),
    };
    const selectedModule = normalizeSelectedModule(operational.store.onboardingSelectedModule) ??
        normalizeSelectedModule(subscription.starterModule);
    const selectedModuleState = selectedModule === "fraud"
