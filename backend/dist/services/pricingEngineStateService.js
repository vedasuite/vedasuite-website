...
exports.derivePricingEngineViewState = derivePricingEngineViewState;
function derivePricingEngineViewState(input) {
    ...
    if (input.timedOutSources.length > 0) {
        return {
            status: "failed_timeout",
            title: "Pricing data took too long to load",
            description: "VedaSuite could not finish loading pricing data in time. Try again in a moment.",
    ...
    if (input.syncStatus === "SYNC_IN_PROGRESS" ||
        ...
        return {
            ...
            description: "VedaSuite is gathering pricing insights from the latest store activity.",
    ...
    if (input.syncStatus === "SYNC_REQUIRED" ||
        ...
        return {
            ...
            description: "More store activity is needed before advanced pricing recommendations are available.",
    ...
    if (input.productsCount === 0) {
        return {
            ...
            description: "Pricing insights will appear after Shopify products are available.",
    ...
    if (input.ordersCount === 0 && input.recommendationCount === 0) {
        return {
            ...
            description: "VedaSuite has product data, but it needs order history before it can make useful pricing recommendations.",
    ...
    if (input.competitorCount === 0) {
        return {
            ...
            description: "Baseline pricing is available. Add competitor websites when you want market comparisons.",
    ...
    return {
        ...
        description: "Pricing analysis completed, but no strong recommendations are available right now.",
