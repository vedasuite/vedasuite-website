...
exports.deriveInstallState = deriveInstallState;
exports.deriveConnectionState = deriveConnectionState;
exports.getMerchantAppState = getMerchantAppState;
...
const storeReadinessService_1 = require("./storeReadinessService");
const subscriptionService_1 = require("./subscriptionService");
function deriveInstallState(health) {
...
function deriveConnectionState(health) {
    if (health.healthy) {
        return {
            status: "healthy",
...
const FALLBACK_STORE_READINESS = {
    billing: {
        plan: "UNKNOWN",
        isActive: false,
        isTrial: false,
...
async function getMerchantAppState(shopDomain) {
    const [health, subscription, billing, onboarding, dashboard, readiness, storeReadiness] = await Promise.all([
        ...
        (0, dashboardService_1.getDashboardMetrics)(shopDomain),
        (0, readinessEngineService_1.getUnifiedReadinessState)(shopDomain).catch((err) => {
            (0, observabilityService_1.logEvent)("error", "app_state.readiness_failed", { shop: shopDomain, error: err });
            return FALLBACK_READINESS;
        }),
        (0, storeReadinessService_1.getStoreReadinessState)(shopDomain).catch((err) => {
            (0, observabilityService_1.logEvent)("error", "app_state.store_readiness_failed", { shop: shopDomain, error: err });
            return FALLBACK_STORE_READINESS;
        }),
    ]);
    if (!dashboard) {
        (0, observabilityService_1.logEvent)("warn", "app_state.dashboard_missing", { shop: shopDomain });
    ...
    const lockedModules = Object.entries(enabledModules)
        ...
        .map(([key]) => key);
    const appStatus = install.status !== "installed" || connection.status === "failed"
        ? "action_required"
        : dashboard.dashboardState.syncHealth.status === "FAILED"
            ? "failed"
            : "ready";
    (0, observabilityService_1.logEvent)("info", "app_state.entitlements_returned", {
        shop: shopDomain,
