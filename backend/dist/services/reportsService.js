...
async function getWeeklyReport(shopDomain) {
    ...
    const [fraudHighRisk, competitorEvents, pricingSuggestions, profitOpportunities, orders, topRiskCustomers, topPricingMoves, topProfitProducts, competitorHighlights,] = await Promise.all([
    ...
    const competitorMomentum = Array.from(competitorByProduct.entries())
        ...
        .slice(0, 5);
    const health = {
        revenueTrend: totalRevenue >= 4000 ? "Strong" : totalRevenue >= 1500 ? "Stable" : "Emerging",
        fraudPressure: fraudHighRisk >= 8 ? "High" : fraudHighRisk >= 3 ? "Medium" : "Low",
        marketPressure: competitorEvents >= 25 ? "High" : competitorEvents >= 10 ? "Medium" : "Low",
        pricingMomentum: pricingSuggestions >= 8 ? "High" : pricingSuggestions >= 3 ? "Medium" : "Low",
    };
