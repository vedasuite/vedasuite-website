...
function maskCustomerLabel(value, fallback = "Shopper profile") {
    if (!value) {
        return fallback;
    }
    if (value.includes("@")) {
...
async function getWeeklyReport(shopDomain) {
    ...
    const [fraudHighRisk, competitorEvents, pricingSuggestions, profitOpportunities, orders, topRiskCustomers, topPricingMoves, topProfitProducts, competitorHighlights, timelineEvents, latestSyncJob,] = await Promise.all([
        ...
        prismaClient_1.prisma.priceHistory.count({
            where: { storeId: store.id, createdAt: { gte: since } },
        }),
        prismaClient_1.prisma.profitOptimizationData.count({
            where: { storeId: store.id, createdAt: { gte: since } },
        }),
        ...
        prismaClient_1.prisma.customer.findMany({
            where: { storeId: store.id },
            orderBy: [{ creditScore: "asc" }, { totalRefunds: "desc" }],
            take: 5,
        }),
        prismaClient_1.prisma.priceHistory.findMany({
            where: { storeId: store.id, createdAt: { gte: since } },
            orderBy: [{ expectedProfitGain: "desc" }],
            take: 5,
        }),
        prismaClient_1.prisma.profitOptimizationData.findMany({
            where: { storeId: store.id, createdAt: { gte: since } },
        ...
        prismaClient_1.prisma.timelineEvent.findMany({
            where: { storeId: store.id, createdAt: { gte: since } },
    ...
    for (const order of orders) {
        ...
        if (order.fraudRiskLevel === "High")
            bucket.fraudHighRisk += 1;
        if (order.refunded || order.refundRequested)
            bucket.refunds += 1;
    }
    const totalRevenue = Number(orders.reduce((sum, order) => sum + order.totalAmount, 0).toFixed(2));
    const totalRefunds = orders.filter((order) => order.refunded || order.refundRequested).length;
    ...
    const competitorMomentum = Array.from(competitorByProduct.entries())
        ...
        priceDelta: bucket.latestPrice != null && bucket.earliestPrice != null
        ...
        revenueTrend: orders.length === 0
            ...
            : totalRevenue >= 4000
                ...
                : totalRevenue >= 1500
                    ? "Stable"
                    : "Emerging",
        fraudPressure: fraudHighRisk >= 8 ? "High" : fraudHighRisk >= 3 ? "Medium" : "Low",
        marketPressure: competitorEvents >= 25 ? "High" : competitorEvents >= 10 ? "Medium" : competitorEvents > 0 ? "Low" : "Awaiting competitor data",
        pricingMomentum: pricingSuggestions >= 8 ? "High" : pricingSuggestions >= 3 ? "Medium" : pricingSuggestions > 0 ? "Low" : "Awaiting pricing data",
    };
    const recommendations = [];
    if (syncState.status === "NOT_CONNECTED") {
        recommendations.push("Reconnect Shopify before reviewing weekly reports so VedaSuite can validate the store installation.");
    }
    else if (syncState.status === "SYNC_REQUIRED") {
        recommendations.push("Update store insights so VedaSuite can build the weekly report from Shopify activity.");
    }
    else if (syncState.status === "SYNC_IN_PROGRESS") {
        recommendations.push("Store insights are updating. Refresh this report shortly to review the latest activity.");
    }
    else if (syncState.status === "SYNC_COMPLETED_PROCESSING_PENDING") {
        recommendations.push("Store activity is being analyzed for pricing, trust, and profit insights.");
    }
    else if (fraudHighRisk > 0) {
        recommendations.push("Review the high-risk fraud queue before approving fulfillment for flagged orders.");
    }
    else if (orders.length === 0) {
        recommendations.push("More Shopify order activity is needed before fraud and abuse guidance appears.");
    }
    else {
        recommendations.push("Fraud pressure is currently low; continue reviewing new orders and refund requests.");
    }
    if (competitorEvents >= 10) {
        recommendations.push("Competitor movement is elevated; review promotion clusters before broad discounting.");
    }
    else if (competitorEvents > 0) {
        recommendations.push("Competitor activity is light; watch the latest market moves before reacting.");
    }
    else {
        recommendations.push("Add competitor domains and run ingestion to unlock live competitor reports.");
    }
    if (profitOpportunities > 0) {
        recommendations.push("Use the Pricing & Profit engine on top-selling SKUs to capture the identified margin lift.");
    }
    else if (pricingSuggestions > 0) {
        recommendations.push("Pricing recommendations are available, but profit opportunities are still limited.");
    }
    else {
        recommendations.push("Sync orders and products first so VedaSuite can generate pricing and profit guidance.");
    }
    return {
        since,
        summary: {
