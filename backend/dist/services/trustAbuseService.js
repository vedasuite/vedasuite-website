...
exports.getTrustAbuseOverview = getTrustAbuseOverview;
...
function maskIdentity(value, fallback) {
    return value ? (0, merchantLabels_1.maskMerchantCustomerLabel)(value) : fallback;
}
async function getTrustAbuseOverview(shopDomain) {
    const [subscription, fraudOverview, trustLayer, recentOrders, customers, store, operational,] = await Promise.all([
        (0, subscriptionService_1.getCurrentSubscription)(shopDomain),
        (0, fraudService_1.getFraudIntelligenceOverview)(shopDomain),
        (0, creditScoreService_1.getTrustOperatingLayer)(shopDomain),
        (0, fraudService_1.listRecentFraudOrders)(shopDomain),
        (0, creditScoreService_1.listCustomerScores)(shopDomain),
    ...
    const behaviorTimeline = store.timelineEvents.length > 0
                ...
                tier: typeof metadata.category === "string"
                    ...
                    : event.severity === "critical"
                        ...
                        : event.severity === "success"
                            ? "Trusted Buyer"
                            : "Standard Buyer",
                refundRate: typeof metadata.refundRate === "number"
        ...
        : customers.slice(0, 6).map((customer) => ({
            ...
            eventSummary: customer.creditScore >= 80
    ...
    const likelyRefundChannel = trustLayer.segments.risky > trustLayer.segments.trusted
    ...
    const trustRecoveryActions = [
        {
            ...
            detail: "Guide borderline shoppers toward exchanges or store credit before full cash refunds when return-abuse pressure is rising.",
            eligibleProfiles: trustLayer.segments.normal,
            priority: trustLayer.segments.normal >= 3 ? "High opportunity" : "Monitor",
        },
        {
            title: "Protect the high-trust lane",
            detail: "Keep the best shoppers in a low-friction support flow so the trust score remains a merchant advantage.",
            eligibleProfiles: trustLayer.segments.trusted,
            priority: trustLayer.segments.trusted > 0 ? "Operationalize now" : "Seed more data",
        },
        {
            title: "Escalate repeat abuse patterns",
            detail: "Customers with refund-heavy histories or repeat fraud signals should be routed into manual review until trust recovers.",
            eligibleProfiles: trustLayer.segments.risky,
            priority: trustLayer.segments.risky > 0 ? "Requires workflow" : "No current pressure",
        },
    ];
    const smartPolicyRecommendations = [
        {
            name: "Trusted fast lane",
            description: "Auto-approve standard refund and exchange requests for trusted shoppers with low abuse indicators.",
            appliesTo: "Trust score 80+",
            action: "Instant refund or exchange",
        },
        {
            name: "Store-credit protection",
            description: "Route medium-trust shoppers with rising refund frequency toward store credit or exchange-first handling.",
        ...
        },
        {
            ...
            description: "Hold refund and exception requests when return-abuse and fraud signals stack together.",
    ...
    const refundOutcomeOptions = [
        {
            ...
            marginImpact: trustLayer.segments.trusted > trustLayer.segments.risky
                ? "Lower retention, fastest resolution"
                : "Use selectively for the highest-trust lane",
            confidence: trustLayer.segments.trusted > 0 ? "High for trusted buyers" : "Needs more history",
        ...
        },
        {
            ...
            confidence: trustLayer.segments.normal > 0 ? "Strong for medium-trust profiles" : "Requires more mixed trust traffic",
            recommendedWhen: "Medium trust or rising refund frequency",
        },
        {
            channel: "Exchange",
            marginImpact: "Protects revenue and retains the order value",
            confidence: queue.some((order) => !order.refundRequested)
    ...
    const supportCopilotCases = [
        highestRiskOrder
            ? {
                ...
                reason: highestRiskOrder.riskLevel === "High"
                ...
                recommendedHandling: highestRiskOrder.riskLevel === "High"
        ...
        behaviorTimeline[0]
            ? {
                ...
                recommendedHandling: behaviorTimeline[0].trustScore >= 80
    ...
    return {
        ...
        networkMatches: fraudOverview.networkMatches.filter((match) => !!match.orderLabel &&
        ...
        refundOutcomeSimulator: {
            ...
            merchantOutcome: highestRiskOrder && highestRiskOrder.refundRequested
            ...
            recoveryRate: trustLayer.segments.trusted > 0
            ...
            recommendedAction: highestRiskOrder?.riskLevel === "High"
